...
app.config['MONGO_URI'] = 'mongodb+srv://kunalgiri:8149509019k@ams.1qyodxz.mongodb.net/data'
...
def user():
    ...
    if user_data:
        ...
        if user_profile:
            ...
            attendance_data = fetch_attendance_data_for_user(user_name, year, month)
...
def capture():
    try:
        ...
        if recognized_name:
            ...
            existing_record = db.attendance_records.find_one({'name': recognized_name, 'date':today_date, 'status': "Present"})

            if existing_record:
                error_message = f'Attendance already recorded for {recognized_name} today'
